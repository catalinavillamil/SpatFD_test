varfcok=function(SFD,modelo,fixcoords,movcoords){
     ...
     coordstot=rbind(fixcoords,movcoords,SFD[[2]]$coords)
     colnames(coordstot)=c("X","Y")
     #coordenadas2=data.frame("SFD"=coordmov[1:n2],"Y"=coordmov[(n2+1):N2])#ubicar coordenadas
     ...
     diff22=MatDistances[(n1+1):n2,(n1+1):n2]

     diff12=MatDistances[1:n1,((n1+1):n2)]
     ...
     sigma11=variogmatrix11[[i]]-variogmatrix11[[i]]
     for(i in 1:length(sq11)){
     ...
     sigma12=variogmatrix12[[i]]-variogmatrix12[[i]]
     for(i in 1:length(sq12)){
